  ...
  for (int v = 0; v < MACROBLOCK_SIZE; v++) {
    for (int u = 0; u < MACROBLOCK_SIZE; u++) {
      for (int y = 0; y < MACROBLOCK_SIZE; y++) {
        ...
        precalcDct[v][u][y][0] = vmulq_f32(dct_x_vec1, dct_y_vec);
        precalcDct[v][u][y][1] = vmulq_f32(dct_x_vec2, dct_y_vec);


        float32x4_t idct_y_vec = vdupq_n_f32(dctlookup[v][y]);
        precalcIdct[v][u][y][0] = vmulq_f32(*((float32x4_t *) dctlookup[u]), idct_y_vec);
        precalcIdct[v][u][y][1] = vmulq_f32(*((float32x4_t *) &dctlookup[u][4]), idct_y_vec);
      }
    }
  }
}
